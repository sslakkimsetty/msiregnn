...
class BsplineRegistration(tf.keras.models.Model):
    ...
    def __init__(
        ...
        self.resize = tf.keras.layers.Resizing(
            height=self.grid_res[0],
            width=self.grid_res[1]
        )
    ...
    def call(self):
        if not hasattr(self.locnet, "built") or not self.locnet.built:
            self.locnet.build(input_shape = self.moving.shape)
        xs = self.locnet(inputs = self.fixed)

        if not hasattr(self.resize, "built") or not self.resize.built:
            self.resize.build(input_shape = xs.shape)
        xs = self.resize(xs)
        self.theta = tf.transpose(xs, [0, 3, 1, 2])

        if not hasattr(self.transformer, "built") or not self.transformer.built:
            self.transformer.build(input_shape = self.moving.shape)
